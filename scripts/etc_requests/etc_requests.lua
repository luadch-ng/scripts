--[[
    Imported from luadch/scripts (etc_requests_v0.8 (multi)) on 2026-05-05.
    ...
    - 1-arg os.difftime( os.time() - start ) on the onTimer handler replaced
      with direct (os.time() - start). Same family of fix as
      luadch-ng/scripts#6.
    - Typo on the add-release path: user.firstnick() (dot) -> user:firstnick()
      (colon). Method calls on user-objects need self; the dot form would
    ...
    incompatible. New deployments are unaffected.
]]--
