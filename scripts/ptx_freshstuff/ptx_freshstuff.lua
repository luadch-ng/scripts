--[[
    Imported from luadch/scripts (ptx_freshstuff_v0.11 (multi3)) on 2026-05-05.
    ...
    Lua 5.4 / v3.1.x fixes (this fork):
    - Removed module-level setmetatable( getfenv(1), nil ) (getfenv gone).
    - Dropped local table_getn = table.getn alias (table.getn removed in 5.4).
    - Replaced table_getn(tmp) with #tmp at the one call site.
    - Added missing local scriptlang = cfg.get("language") before the
      loadlanguage call. Upstream referenced scriptlang as an
      undeclared identifier; in our strict-env sandbox that crashes
      with "attempt to read undeclared var".
    - Forward-declared seven top-level functions as locals (parsecmds,
      RegCmd, SendTxt, SplitTimeString, JulianDate, frac,
      CreateRightClicks). Upstream wrote them as bare globals via
      function NAME(...) - in our strict-env sandbox global writes
      raise "attempt to write undeclared var". Forward-decl + same
      function NAME(...) syntax assigns to the local.
    - 1-arg os.difftime( os.time() - start ) on the onTimer handler
      replaced with direct (os.time() - start). Same family of fix as
      luadch-ng/scripts#6.
...
-- removed on import: local table_getn = table.getn -- table.getn was removed
-- in Lua 5.4. Use #table directly. The one call site below was patched too.
...
-- forward-declared on import: the seven top-level functions below originally
-- wrote to globals via "function NAME(...)" syntax; in our strict-env sandbox
-- that crashes with "attempt to write undeclared var". Pre-declared here so
-- "function NAME(...)" later assigns to the local instead.
local parsecmds, RegCmd, SendTxt, SplitTimeString, JulianDate, frac, CreateRightClicks

--// msgs
-- added on import: scriptlang was referenced but never defined upstream;
-- under our strict-env sandbox an undeclared read crashes at load.
  ...
  if commandtable[ cmd ] then
    if commandtable[ cmd ][ "level" ] ~= 0 then
      if userlevels[ user:level() ] >= commandtable[ cmd ][ "level" ] then
        commandtable[ cmd ][ "func" ]( user, data, env, table.unpack( commandtable[ cmd ][ "parms" ] ) )
      else
    ...
    if FreshStuff.Types[ cat ] then
      ...
      if FreshStuff.Count > 0 then
        for i = 1, FreshStuff.Count do
          local ct, who, when, title = table.unpack( FreshStuff.AllStuff[ i ] )
          if title == tune then
    ...
    if FreshStuff.Count2 == 0 then
      ...
      for i = 1, FreshStuff.Count2 do
        if FreshStuff.NewestStuff[ i ] then
          cat, who, when, title = table.unpack( FreshStuff.NewestStuff[ i ] )
          if title then
...
function FreshStuff.SearchRel( user, data, env )
  ...
  if what then
    ...
    if #rest ~= 0 then
      for i = 1, #rest do
        local type, who, when, title, nr = table.unpack( rest[ i ] )
        res = res + 1
